...
export async function renderNewsletter(props: NewsletterProps): Promise<{ html: string; text: string }> {
  const html = await render(Newsletter(props));
  const text = await render(Newsletter(props), { plainText: true });
  return { html, text };
}

export async function renderConfirmation(props: ConfirmProps): Promise<{ html: string }> {
