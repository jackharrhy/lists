...
export function Button({ children, variant = "primary", size = "md", ...props }: Record<string, any>) {
  ...
  return <button hx-disabled-elt="this" {...props} class={`inline-block ${base} ${s} ${v} ${props.class ?? ""}`}>{children}</button>;
}
...
export function Td({ children, class: cls, colspan }: { children?: Child; class?: string; colspan?: number }) {
  return <td colspan={colspan} class={`px-4 py-3 border-b border-gray-100 ${cls ?? ""}`}>{children}</td>;
}
