...
import { Button, LinkButton, Input, Select, Table, Th, Td, PageHeader, Pagination } from "./ui";
...
function activeTemplateChoices(db: Db): CampaignTemplateChoice[] {
  return db
    ...
    .where(eq(schema.emailTemplates.status, "active"))
    .all()
    .map((template) => ({
      slug: template.slug,
      name: template.name,
...
export function mountCampaignRoutes(app: App, db: Db, config: Config) {
  ...
  app.get("/campaigns/:id/preview", async (c) => {
    ...
    if (subscriberId) {
      const sub = db
        ...
        .where(eq(schema.subscribers.id, Number(subscriberId)))
        .get();
      if (sub) {
        unsubscribeUrl = buildUnsubscribeUrl(
          config.baseUrl,
          sub.unsubscribeToken,
          (campaign.audienceType === "list" ? campaign.audienceId : undefined) ?? undefined,
        );
  ...
  app.get(
    ...
    (c) => {
      ...
      if (listAccess === "all") {
        ...
        campaigns = (conditions ? q.where(conditions) : q).limit(CAMPAIGNS_PAGE_SIZE).offset(offset).all();
      } else if (listAccess.length === 0) {
        campaigns = [];
        total = 0;
      } else {
        const accessCond = and(
          eq(schema.campaigns.audienceType, "list"),
          inArray(schema.campaigns.audienceId, listAccess),
        );
        ...
        <AdminLayout title="Campaigns" user={user} flash={flash}>
          ...
          >
            ...
            {(filterStatus || filterSearch) && (
              <a href="/admin/campaigns" class="text-sm text-gray-500 hover:text-gray-700 no-underline">
                Clear
              </a>
            )}
          </form>
  ...
  app.get("/campaigns/new", (c) => {
    ...
    return c.html(
      <CampaignEditorPage
        ...
        templates={activeTemplateChoices(db)}
      />,
    );
  });
  ...
  app.get("/campaigns/:id", async (c) => {
    ...
    if (
      ...
      listAccess !== "all" &&
      !listAccess.includes(campaign.audienceId)
    ) {
      ...
      <AdminLayout title={campaign.subject} user={user} flash={flash}>
        ...
        {campaign.scheduledAt && (
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 text-sm text-blue-800">
            ...
            {campaign.batchSize && (
              <span class="ml-4">
                Batch: {campaign.batchSize} emails every {campaign.batchInterval ?? 10} minutes
              </span>
            )}
          </div>
        )}
        ...
        {campaign.status === "failed" && (
          <div class="flex gap-2 mb-6">
            ...
            <form method="post" action={`/admin/campaigns/${id}/reset`}>
              <button
                type="submit"
                class="inline-block px-4 py-2 bg-gray-500 text-white text-sm font-medium rounded-md hover:bg-gray-600 cursor-pointer border-none no-underline"
              >
                Reset to Draft
              </button>
            </form>
                ...
                {sends.map((send) => (
                  <tr>
                    ...
                    <Td>
                      ...
                      {send.deliveredAt && (
                        <span class="block text-xs text-green-700">Delivered {fmtDateTime(send.deliveredAt)}</span>
                      )}
                      {send.nextAttemptAt && (
                        <span class="block text-xs text-amber-700">Retry {fmtDateTime(send.nextAttemptAt)}</span>
                      )}
                      {send.lastError && (
                        <span class="block text-xs text-red-700" title={send.diagnosticCode ?? undefined}>
                          {send.lastError}
                        </span>
                      )}
                    </Td>
        ...
        <div class="mb-4">
          <select
            id="previewSubscriber"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm font-[inherit] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
              ...
              <tbody>
                {inboundReplies.map((r) => (
                  <tr>
                    ...
                    <Td>
                      <a href={`/admin/inbound/${r.id}`} class="text-blue-600 hover:text-blue-800">
                        View
                      </a>
                    </Td>
                  </tr>
        ...
        <form
          method="post"
          action={`/admin/campaigns/${id}/delete`}
          onsubmit="return confirm('Delete this campaign and all its send records? This cannot be undone.')"
        >
          <Button type="submit" variant="danger">
            Delete Campaign
          </Button>
  ...
  app.post("/campaigns/:id/reset", (c) => {
    const id = Number(c.params.id);
    db.update(schema.campaigns).set({ status: "draft", lastError: null }).where(eq(schema.campaigns.id, id)).run();
  ...
  app.post("/campaigns/:id/unschedule", (c) => {
    const id = Number(c.params.id);
    db.update(schema.campaigns).set({ status: "draft", scheduledAt: null }).where(eq(schema.campaigns.id, id)).run();
  ...
  app.post("/campaigns/:id/delete", async (c) => {
    ...
    db.transaction((tx) => {
      ...
      tx.update(schema.messages).set({ campaignId: null }).where(eq(schema.messages.campaignId, id)).run();
      tx.update(schema.events).set({ campaignId: null }).where(eq(schema.events.campaignId, id)).run();
      tx.delete(schema.campaignSends).where(eq(schema.campaignSends.campaignId, id)).run();
