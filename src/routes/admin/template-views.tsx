...
function TemplatePreview({ template }: { template: Template }) {
  return <div class="h-32 overflow-hidden rounded-lg border border-gray-200 bg-white" aria-hidden="true">
...
export function TemplateGalleryPage({ user, templates }: { user: User; templates: Template[] }) {
  return <AdminLayout title="Email templates" user={user}>
    <PageHeader title="Email templates"><span class="text-sm font-normal text-gray-500">{templates.length} template{templates.length === 1 ? "" : "s"}</span></PageHeader>
    <p class="-mt-4 mb-6 max-w-2xl text-sm text-gray-600">Preview the layouts available to campaigns. Use MCP or the API to create and update them.</p>
    {templates.length === 0
      ? <Card><p class="m-0 py-5 text-center text-sm text-gray-500">No templates yet.</p></Card>
      : <div class="mb-6 overflow-hidden rounded-lg border border-gray-200 bg-white">{templates.map((template) => {
        const sections = JSON.parse(template.sections) as TemplateSection[];
        return <a href={`/admin/templates/${template.slug}`} class="group grid gap-4 border-b border-gray-200 px-5 py-5 text-inherit no-underline last:border-b-0 md:grid-cols-[12rem_minmax(0,1fr)_auto] md:items-center">
...
function PreviewButton({ value, children, active = false }: { value: string; children: string; active?: boolean }) {
  return <button type="button" data-preview-mode={value} aria-pressed={String(active)} class={`rounded border border-gray-300 px-2.5 py-1 text-xs ${active ? "bg-gray-900 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>{children}</button>;
}
...
export function TemplateWorkspacePage({ user, template, sources }: { user: User; template: Template; sources: HighlightedTemplateSources }) {
  ...
  return <AdminLayout title={template.name} user={user}>
    ...
    <header class="mb-6 mt-4">
      ...
      <p safe class="-mt-4 mb-0 max-w-2xl text-sm text-gray-600">{template.description || "No description."}</p>
    </header>
    <Card>
      ...
      <div class="mb-3 flex flex-wrap items-center gap-2">
        ...
        {[375, 600, 800].map((width) => <button type="button" data-preview-width={String(width)} aria-pressed="false" class="rounded border border-gray-300 bg-white px-2.5 py-1 text-xs text-gray-600 hover:bg-gray-50">{width} px</button>)}
        <button type="button" data-preview-width="full" aria-pressed="true" class="rounded border border-gray-900 bg-gray-900 px-2.5 py-1 text-xs text-white">Full width</button>
      ...
      <div class="flex min-h-[680px] justify-center overflow-auto rounded-lg border border-gray-200 bg-gray-100 p-4"><iframe title={`${template.name} preview`} sandbox="" data-template-preview-frame src={previewPath} class="min-h-[640px] w-full rounded bg-white transition-[width]" /></div>
      </section>
    </Card>
    <Card>
      <section>
