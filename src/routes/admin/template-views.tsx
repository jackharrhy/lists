...
export function TemplateWorkspacePage({ user, template, sources }: { user: User; template: Template; sources: HighlightedTemplateSources }) {
  ...
  return <AdminLayout title={template.name} user={user}>
    ...
    <section class="mt-6 space-y-3"><details class="rounded-xl border border-gray-200 bg-white" open><summary class="cursor-pointer px-5 py-4 text-sm font-semibold">{template.sourceFormat.toUpperCase()} source</summary><div class="template-source">{sources.html}</div></details><details class="rounded-xl border border-gray-200 bg-white"><summary class="cursor-pointer px-5 py-4 text-sm font-semibold">Plain-text source</summary><div class="template-source">{sources.text}</div></details>{Object.keys(partials).map((name) => <details class="rounded-xl border border-gray-200 bg-white"><summary class="cursor-pointer px-5 py-4 text-sm font-semibold">Partial: <code>{name}</code></summary><div class="template-source">{sources.partials[name]}</div></details>)}</section>
  </AdminLayout>;
}
