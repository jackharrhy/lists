...
export function ingestDmarcEmail(db: Db, payload: DmarcInboundPayload, parsedEmail: ParsedMail, s3Key: string) {
  const existing = db.select().from(schema.dmarcIngestions)
    .where(eq(schema.dmarcIngestions.sesMessageId, payload.messageId)).get();
  if (existing?.status === "parsed" || existing?.status === "rejected") return existing;

  const ingestion = existing ?? db.insert(schema.dmarcIngestions).values({
  ...
  try {
    const attachment = findDmarcAttachment(parsedEmail.attachments.map((item) => ({
    ...
    const savedReport = db.transaction((tx) => {
      let stored: typeof schema.dmarcReports.$inferSelect | undefined = tx.insert(schema.dmarcReports).values({
        reportKey,
        ...
        messageCount,
      }).onConflictDoNothing({ target: schema.dmarcReports.reportKey }).returning().get();

      if (stored) {
        tx.insert(schema.dmarcReportRecords).values(report.records.map((record) => ({
          reportId: stored!.id,
        ...
        stored = tx.select().from(schema.dmarcReports)
          .where(eq(schema.dmarcReports.reportKey, reportKey)).get();
      }

      if (!stored) throw new Error("DMARC report could not be stored");
      tx.update(schema.dmarcIngestions).set({
        status: "parsed",
        reportId: stored.id,
        error: null,
        processedAt: new Date().toISOString(),
      }).where(eq(schema.dmarcIngestions.id, ingestion.id)).run();
      return stored;
    });
    ...
    db.update(schema.dmarcIngestions).set({
      status: "rejected",
      error: message,
      processedAt: new Date().toISOString(),
    }).where(eq(schema.dmarcIngestions.id, ingestion.id)).run();
