...
export function rescheduleInterruptedCampaigns(db: Db, now = new Date()) {
  const interrupted = db.select({ id: schema.campaigns.id }).from(schema.campaigns)
    .where(eq(schema.campaigns.status, "sending")).all();
  if (interrupted.length === 0) return 0;
  db.update(schema.campaigns).set({
    status: "scheduled",
  ...
  }).where(eq(schema.campaigns.status, "sending")).run();
...
export function recoverAbandonedDeliveries(db: Db, now = new Date()) {
  ...
  const abandoned = db.select({ id: schema.campaignSends.id }).from(schema.campaignSends)
    .where(and(
      inArray(schema.campaignSends.status, ["pending", "attempting"]),
      lte(schema.campaignSends.updatedAt, cutoff),
    )).all();
