...
const pendingImages = z.string().default("{}").transform((value, context): Record<string, string> => {
  try {
...
export const CampaignEditorFormSchema = z.object({
  fromAddress: z.string().trim().min(1),
  fromName: z.string().trim().default("").transform((value) => value || null),
  ...
  scheduledAt: z.string().default("").transform((value, context) => {
  ...
  if (form.audienceMode === "list") {
    ...
    const ids = [...new Set(form.subscriberIds.split(",").map(Number).filter((id) => Number.isInteger(id) && id > 0))];
...
function assertAudienceAccess(db: Db, user: { id: number; role: string }, audience: CampaignAudience) {
  ...
  if (audience.type === "subscribers") {
    const rows = db.select({ id: schema.subscribers.id }).from(schema.subscribers)
      .where(and(inArray(schema.subscribers.id, audience.ids), eq(schema.subscribers.status, "active"))).all();
    if (rows.length !== audience.ids.length) throw new CampaignEditorReferenceError("One or more subscribers are unavailable");
  }
}
